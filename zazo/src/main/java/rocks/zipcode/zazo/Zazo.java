...
public class Zazo extends JPanel implements TreeSelectionListener, DocumentListener {
    ...
    /*
     * This main is an example of how Threads are used in Java.
     * The main thread is used to create the GUI, and the GUI is run on the Event Dispatch Thread.
     * This is a common pattern in Java GUI programming.
     * See the `new Runnable()` call below. The `run()` method is 
     * called on the Event Dispatch Thread.
     * 
     * When build java apps that use a graphic UI (as opposed to a command line interface),
     * javax.swing is often used. Integrating the program into the event loop system that most
     * OSs have running their window systems, is a complex task. The Event Dispatch Thread is
     * a single thread that is responsible for handling all events that are generated by the
     * user interface components. This is why it is important to keep the Event Dispatch Thread
     * free from any long running tasks, as it will block the UI from updating.
     * 
     */
...
/* 
 * JavaFX is a newer GUI toolkit that is part of the Java standard library.
 * It is more modern and has more features than Swing, but Swing is still
 * widely used in many Java applications.
 */